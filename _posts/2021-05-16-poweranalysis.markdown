...
중심극한정리는 통계를 쉬워 보이게 도와줍니다. `적당히 큰 N에 대해서` 분포들의 평균은 언제나 정규분포를 따른다. 그러니깐  p-value든 z socre든, 대충 정규분포를 가정하여 계산하고 검정에 사용하라고 말해줍니다. 그런데 정말 그럴까요? 미신처럼 내려오는 마법의 샘플 숫자 30개.. 믿어도 될까요?  
...
**검정력**은 귀무가설이 틀렸을 때, 귀무 가설이 틀렸다고 말 할 확률입니다.  
...
A priori power-analysis는 유의 수준, 검정력, Effect Size가 주어졌을 때, 샘플 사이즈 N을 추정하는 분석입니다.  
...
우리의 목표인 적절한 Sample Size N을 구하는 문제를 하나 풀어보겠습니다.  

> 상자안에 파란 구슬과 빨간 구슬들이 들어가있다.  
>
> 파란 구슬을 꺼낼 확률이 모든 시도에서 0.1보다 작다고 기대하고 있다.  
>
> 그런데 알고보니 실제로는 파란 구슬을 꺼낼 확률이 **0.2**였다.  
>
> 이런 경우에, **0.01** 유의 수준내에서, 파란 구슬을 꺼낼 확률이 0.1보다 크다고  
>
> 말 할 확률이 **0.8** 이상이었으면 좋겠다.  
>
...
>
> - 귀무가설 기각의 기준이 되는 유의수준 alpha는 0.01이다.  
>
...
(조건 1) Power는 실제로 귀무가설이 틀렸을 때, 틀렸다고 말 할 확률입니다.  

&#8594; Power는 대립가설이 맞았을 때, 귀무가설을 기각 할 확률입니다.  

&#8594; Power = P(reject 귀무가설 | 대립가설 is true)  

- 지금 우리의 문제에서 대립가설은 p = 0.2입니다.  

우선은 위 이미지와 같이 유의수준 0.01내에서 귀무가설을 기각시키는 표본 평균의 범위를 구합니다.  

귀무가설을 기각하기 위해서는 z-value가 임계값인 1.96보다 큰 값이어야 합니다.  
...
&#8594; P( p < 조건1을 만족시키는 p' | p = 0.2) = type2 error  

&#8594; P( z < (p'-0.2) / sd) = type2 error  

type2 error가 0.2보다 작아야 하기 때문에 (p' - 0.2) / sd는 신뢰수준 80%의 z-value인 -1.282보다 작아야 합니다.  
...
조건1,2를 모두 만족하는 N의 범위의 최소값이 모든 조건을 만족시키는 최소의 Sample Size라고 볼 수 있습니다.  
