...
Shiny는 반응형의 웹 페이지를 쉽게 만들 수 있게 도와주는 R 패키지다. 제작년에 정부 교육을 들을 때 프로젝트로 만들어 볼 기회가 있었는데, html이나 css 같은 것들을 아무것도 모르는 상태로도 웹 어딘가에 나의 결과물을 배포할 수 있다는 것이 신기했다. 물론 디자인이나 기능적인 측면에서의 자유도는 아직 제약이 좀 있는 것 같지만 `통계 분석 결과물을 시각적으로 쉽게 공유하자` 취지에서는 충분하다 생각한다. 아주 간단하게만 어떻게 Shiny가 돌아가는지 살펴보자.
...
<img src="/assets/img/wt/dbutil/dbutiltwo.jpg">  
...
다시 Server 모듈로 돌아와보자. R에서 데이터 프레임의 컬럼은 df$[컬럼 이름]의 형태로 호출할 수 있다. 이것과 비슷하게 우리가 ui에서 정의해 준 여러 이름들은 intput$[이름]의 형태로 저장되어 있다. output$[이름]도 물론 마찬가지이다. 그런데 output의 경우 이름은 붙여줬지만, 막상 어떤 모습으로 나올지는 ui에서 정의해준 것이 없다. 이것은 server에서 아래와 같이 정의해주면 된다.

>  output$[이름] <- (우리가 정의한 아웃풋)  

그런데 mainPanel에서 이미 plotOutput()이라는 도구로 와꾸를 정의해줬기 때문에, output은 오직 plot 형태의 결과물로 제한된다. 예제에서는 히스토그램을 사용했고, intput$bins 인풋을 사용해서 Plot을 그려줬다. 여기까지 구성해준다면, (1)틀을 짜줬고 (2)반응형의 이름들을 정의해줬고 (3)정의된 이름들을 사용해서 모듈도 구성해준 것이다. 그럼 마지막으로 정의한 ui와 server 함수를 `shinyApp(ui, server)` 함수로 실행만 시켜주면 끝이다.  
