...
seo:
  date_modified: 2020-04-25 13:20:52 +0900
...
로스가 줄어드는 것만 보면 굉장히 최적화 된 것 같다. 이어서 MAML에게 기대하는 Few - Shot Learning을 얼만큼 잘 수행할 수 있는지 확인해보자.
...
pretrained = preTrain(1000,window,1)
```
...
def loss_on_new_task(initial_model,task,num_steps,K,in_features,out_features,optim=torch.optim.SGD):
    ...
    losses = []
    X = torch.stack([a[-(b+1)][0] for a in task for b in range(K)])
    y = torch.stack([a[-(b+1)][1] for a in task for b in range(K)])
...
task = [tasks[x] for x in sample(list(tasks.keys()),100)]
for idx in range(100):
  maml_losses.append(loss_on_new_task(maml.model.fc,task, 10, 10, 30, 1))
  pretrain_losses.append(loss_on_new_task(pretrained,task, 10, 10, 30, 1))
