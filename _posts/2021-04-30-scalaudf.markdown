...
2. Array of Array Column 두 번째 값을 기준으로 정렬 해주기
...
데이터는 제가 최근 쳐물려있는 주식 종목들의 (종목 코드, 최근 5일 시세, 기업 개요) 로 하도록 하겠습니다.  
...
def minMaxScaler(Inp: Seq[Double]): Seq[Double] = {
  ...
  if (max_Inp = min_Inp) {
    ...
    // 조금 멍청한 방법인 것 같은데요, 우선 분자 부분의 (v - min)을 수행해주고, 
