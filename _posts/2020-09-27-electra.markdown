...
이외에 여러 임베딩 기법이라던가 실험적인 부분들은 패스하고 오늘 리뷰하고자 하는 ELECTRA로 넘어가보자.
...
class DynamicNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        randint = range(random.randint(0, 3))
        print(randint)
...
> DynamicNet은 한 번 Input이 들어갈때마다  
>
> input layer - (1~3개 중 랜덤으로 한 사이즈의) middle layer - output layer를 거친다.
>
> 만약 동일한 크기의 로스가 발생한다면 middle layer가 두 겹일 때라 한 겹일 때가 다르게 업데이트 될까?
...
<center><small>모델은 정직하게 세 개의 Layer를 가진 형태로 보여진다.</small></center>

```python
model_test1 = type(model)(D_in, H, D_out)
model_test1.load_state_dict(model.state_dict())

model_test2 = type(model)(D_in, H, D_out)
model_test2.load_state_dict(model.state_dict())
```

> 실험을 위해 model을 카피한 두 개의 Test 모델을 생성한다.
